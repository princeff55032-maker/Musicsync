#!/usr/bin/env bash
#
# update.sh - Pull, build, and restart the musicsync server
#
set -euo pipefail
...
warn "Restarting PM2"
pm2 restart musicsync-server 2>/dev/null || pm2 start pm2.config.js
ok "PM2 restarted"
...
if curl -sf --max-time 5 http://localhost:8080/ > /dev/null; then
  ...
  pm2 logs musicsync-server --lines 20 --nostream
  exit 1
fi

pm2 show musicsync-server | grep -E "status|uptime|restarts"
ok "Deploy complete"